//! The agenda device pull assembles per-page ink from a multi-page `.rmdoc` in
...
//! slot (no index shift). No device / no rmapi.
...
use agenda::serve::strokes_by_page;
...
fn write_rmdoc(device: &Remarkable, page_h: f64, per_page: &[Vec<Stroke>]) -> std::path::PathBuf {
    ...
    let path = std::env::temp_dir().join(format!("agenda-multipage-{unique}.rmdoc"));
