...
fn collect_sources_includes_header_typst() {
    let doc: Document<()> =
        Document::keyed("d", flow![Notice::line("body")]).page_header(Notice::line("hd"));
    let sources = collect_typst_sources(&doc);
    assert!(
        sources.iter().any(|(p, _)| p == "/inkapp/region.typ"),
        "prelude present"
    );
...
fn header_regions_appear_on_every_page() {
    ...
    let body_lines: Vec<String> = (0..120)
        .map(|i| format!("Line of body content {i}"))
        .collect();
    ...
    let geom = inkapp_core::geometry::PageGeom {
        w: 200.0,
        h: 150.0,
        margin: 8.0,
    };
    ...
    assert!(
        ...
        "test fixture must span ≥2 pages; got {}",
        compiled.pages.len()
    );

    let manifest: Manifest = recover_regions(&compiled).unwrap();
    let hd_regions: Vec<_> = manifest
        .regions
        .iter()
        .filter(|r| r.name == "hd-mark")
        .collect();
    assert!(
