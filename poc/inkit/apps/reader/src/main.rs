//! Assemble and run the reader app from configuration.
//!
...
use reader::{update, view, App, AppConfig, Connectors};
    ...
    match args.cmd {
        ...
        Some(Cmd::Sync) => {
            ...
            let cycle = inkapp::sync_once(&mut application, transport.as_ref())
                ...
                .expect("sync_once");
            println!(
                "reader[{instance}]: synced {} msg(s), {} op(s)",
                cycle.decoded.len(),
                cycle.ops.len()
            );
        }
        Some(Cmd::Run { interval }) => {
            ...
            let transport =
                inkapp::resolve_transport(&device.backend, app_cfg.device_folder.clone(), &secrets)
                    .expect("resolve device transport");
            println!(
                "reader[{instance}]: serving every {secs}s on {} ({})",
        ...
        None => {
            // Default behavior: publish to device.
            let store = ConfigStore::open(&cfg_path).expect("open config");
            let device: DeviceConfig = store.resolve(&instance).expect("resolve device config");
            let app_cfg: AppConfig = store.resolve(&instance).expect("resolve app config");
            let secrets = SecretStore::open_default().expect("open secrets");
...
async fn build_app(
    ...
    instance: &str,
) -> inkapp_core::runtime::App<App, reader::Msg, Connectors> {
    let store = ConfigStore::open(cfg_path).expect("open config");
    let app_cfg: AppConfig = store.resolve(instance).expect("resolve app config");
...
async fn run_doctor(
    ...
    let mut checklist = inkapp::doctor::Checklist::new(secrets_path)
        ...
        .config_resolves::<AppConfig>(&store, instance, "app.reader")
