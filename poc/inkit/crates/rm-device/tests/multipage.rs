//! The device pull assembles per-page ink from a multi-page `.rmdoc` in
//! `.content` order: ink on page k attributes to page k, a passage split across
//! page breaks stitches to one message, and an un-inked middle page stays an empty
//! slot (no index shift). Framework-level (was duplicated in each app's
//! `tests/multipage.rs`); exercises `rm_device::strokes_by_page`. No device / no rmapi.
...
use rm_device::{strokes_by_page, Remarkable};

/// Local test messages (this test exercises the pull path, not an app's view).
...
fn write_rmdoc(device: &Remarkable, page_h: f64, per_page: &[Vec<Stroke>]) -> std::path::PathBuf {
    ...
    let path = std::env::temp_dir().join(format!("rmdev-multipage-{unique}.rmdoc"));
